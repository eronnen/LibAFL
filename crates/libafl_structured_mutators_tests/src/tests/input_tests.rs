...
fn test_count_data_nested() {
    ...
    assert_eq!(nested.count_data::<u8>(), 2);
    // Count u16 fields - should find 1 (x.b)
    assert_eq!(nested.count_data::<u16>(), 1);
    // Count u32 fields - should find 1 (x.c)
    assert_eq!(nested.count_data::<u32>(), 1);
    // Count SimpleStruct - should find 1 (x)
    assert_eq!(nested.count_data::<SimpleStruct>(), 1);
}
...
fn test_sample_data_nested() {
    ...
    assert_eq!(nested.sample_data::<SimpleStruct>(1), None);
}
...
fn test_complexity() {
    ...
    assert_eq!(nested.complexity(), 1 + 1 + simple.complexity());
}
